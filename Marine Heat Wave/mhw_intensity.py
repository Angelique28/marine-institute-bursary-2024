...
intensity_file = r"C:\Users\acheryl\Desktop\mhw_series\mhw_intensity\intensity_data.csv"
...
plt.plot(x1, y1, color='darkgrey', linestyle='-')
plt.plot(x2, y2, color='darkgrey', linestyle='-')
plt.plot(x3, y3, color='darkgrey', linestyle='-')
plt.plot(x4, y4, color='darkgrey', linestyle='-')
plt.plot(x5, y5, color='darkgrey', linestyle='-')
plt.plot(x6, y6, color='darkgrey', linestyle='-')
plt.plot(x7, y7, color='darkgrey', linestyle='-')
plt.plot(x8, y8, color='darkgrey', linestyle='-')
plt.plot(x9, y9, color='darkgrey', linestyle='-')
...
plt.ylabel('Maximum Intensity (°C)', fontsize=14)
plt.title('Marine Heat Wave Event Maximum Intensity levels - Dumanus Bay')
plt.legend()
plt.tight_layout()
plt.show()
...
# Linear Regression for MHW intensity
...
for i, (x, y) in enumerate(zip(x_variables, y_variables)):
    ...
    if len(x_clean) > 1:  # Ensure there is more than one data point
        ...
        if p_value < 0.05:
            print(f'The regression for {sites[i]} intensity is significant')
        else:
            print(f'The regression for {sites[i]} intensity is not significant')
